...
export async function downloadDocument(url: string, doc: AldimDocument) {
  ...
  if (!response.ok) throw new Error("Belge indirilemedi. Tekrar dene.");
  const bytes = await response.arrayBuffer();
  const isPdf = doc.storagePath?.toLowerCase().endsWith(".pdf");
  let output: Uint8Array | ArrayBuffer = bytes;
  if (!isPdf) {
    ...
    const size = image.scaleToFit(page.getWidth() - 48, page.getHeight() - 48);
  ...
  link.href = local;
  link.download = doc.name.replace(/\.[^.]+$/, "") + ".pdf";
  link.click();
