...
export function validateItem(
  item: Pick<
    AldimItem,
    "title" | "category" | "price" | "purchaseDate" | "fields"
  >,
) {
  ...
  for (const f of CATEGORIES[item.category].fields) {
    ...
    if (
      ...
      throw new Error(`${f.label}: geçerli bir tarih gir.`);
  }
}
export function buildReminders(
  ...
  for (const record of item.serviceRecords) {
    if (record.nextFollowUpDate && record.status !== "resolved")
      specs.push({
        type: "service_follow_up",
        fieldKey: `service:${record.id}`,
        titleTemplate: `${record.company} · Servis takibi`,
        bodyTemplate: "",
        notifyBeforeDays: [7, 1],
      });
  }
  return specs.flatMap((spec) => {
    const value = spec.fieldKey.startsWith("service:")
      ? item.serviceRecords.find((s) => s.id === spec.fieldKey.slice(8))
          ?.nextFollowUpDate
      : spec.fieldKey === "purchaseDate"
        ? item.purchaseDate
        : item.fields[spec.fieldKey];
    if (typeof value !== "string" || !isDate(value)) return [];
    const old = previous.find(
      (r) =>
        r.itemId === item.id &&
        r.sourceField === spec.fieldKey &&
        r.dueDate === value,
    );
    return [
      {
        ...
        sourceField: spec.fieldKey,
        dueDate: value,
        notifyBeforeDays: [...new Set([...spec.notifyBeforeDays, 0])],
        notificationIds: old?.notificationIds ?? [],
...
export function nextPaymentDate(
  ...
  if (item.category === "home_bill" && item.fields.recurring === true)
    return addMonthsIso(
      dueDate,
      item.fields.recurrencePeriod === "bimonthly"
        ? 2
        : item.fields.recurrencePeriod === "quarterly"
          ? 3
          : 1,
    );
  return null;
}
export const itemAmount = (item: AldimItem): number =>
  Number(
    item.category === "subscription"
      ? (item.fields.monthlyAmount ?? item.price ?? 0)
      : (item.fields.amount ?? item.price ?? 0),
  );
export function spendingSummary(
  ...
  return {
    total:
      items
        .filter((i) => !["subscription", "home_bill"].includes(i.category))
        .reduce((n, i) => n + (i.price ?? 0), 0) +
      payments.reduce((n, p) => n + p.amount, 0),
    thisMonth:
      items
        .filter(
          (i) =>
            !["subscription", "home_bill"].includes(i.category) &&
    ...
    subscriptions: active
      ...
      .reduce(
        (n, i) =>
          n +
          itemAmount(i) /
            (i.fields.billingCycle === "yearly"
              ? 12
              : i.fields.billingCycle === "quarterly"
                ? 3
                : 1),
        0,
      ),
    ...
    warranties: active.filter(
      (i) =>
        typeof i.fields.warrantyEndDate === "string" &&
        daysUntil(i.fields.warrantyEndDate) >= 0,
    ).length,
  };
}
