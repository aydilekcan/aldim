...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const signIn = useCallback<AuthApi["signIn"]>(async (email, password) => {
    if (!supabase)
      return { ok: false, error: "Giriş sistemi yapılandırılmamış." };
  ...
  const signUp = useCallback<AuthApi["signUp"]>(async (email, password) => {
    ...
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: { emailRedirectTo: "https://aldim.vercel.app/app" },
    });
    if (error) return { ok: false, error: translateError(error.message) };
    return { ok: true };
  }, []);
  ...
  const signOut = useCallback<AuthApi["signOut"]>(async () => {
    if (supabase) {
      ...
      if (data.user) await unregisterPushDevice(data.user.id);
  ...
  const resetPassword = useCallback<AuthApi["resetPassword"]>(async (email) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: "https://aldim.vercel.app/auth/reset",
    });
    if (error) return { ok: false, error: translateError(error.message) };
    return { ok: true };
  }, []);
