...
export default function SettingsScreen() {
  ...
  const run = async (action: () => Promise<unknown>, message?: string) => {
    ...
    try {
      await action();
      if (message) Alert.alert("Tamam", message);
    } catch (e) {
      Alert.alert("İşlem tamamlanamadı", (e as Error).message);
    } finally {
      setBusy(false);
    }
  };
  return (
    <ScrollView
      contentContainerStyle={{ padding: 20, gap: 18, paddingBottom: 45 }}
    >
      <Card>
        <CardTitle>Hesabın</CardTitle>
        <Text style={{ color: colors.ink[500], marginVertical: 12 }}>
          {user?.email}
        </Text>
        <Text
          style={{
            color: store.syncError ? colors.danger[600] : colors.brand[700],
            marginBottom: 14,
          }}
        >
        ...
        <Button
          ...
          variant="outline"
          loading={busy}
          onPress={() => void run(store.syncNow)}
        />
      </Card>
      <Card>
        ...
        {[
          { key: "notificationsEnabled", title: "Telefon bildirimi" },
          { key: "emailEnabled", title: "E-posta" },
          { key: "smsEnabled", title: "SMS" },
        ].map(({ key, title }) => (
          <View
            key={key}
            style={{
          ...
          >
            <Text style={{ fontSize: 15, color: colors.ink[900] }}>
              {title}
            </Text>
            <Switch
              ...
              onValueChange={async (value) => {
                if (
                  ...
                  Alert.alert(
                    ...
                    "Telefon ayarlarından Aldım bildirimlerini açabilirsin.",
                    [
                      { text: "Vazgeç" },
                      {
                        text: "Ayarları aç",
        ...
        >
          Mobil bildirim kullandığında telefonuna hatırlatırız. Diğer durumlarda
          seçtiğin e-posta ve SMS kanallarını kullanırız. Teslim için gönderim
          hizmetinin etkin olması gerekir.
        </Text>
        {settings.smsEnabled && (
          <Field label="Telefon">
            <TextField
      ...
      <Card>
        <CardTitle>Verilerin senin</CardTitle>
        <Text
          style={{ color: colors.ink[500], lineHeight: 20, marginVertical: 15 }}
        >
          Kayıtlarını ve ödeme geçmişini dışa aktar. Belge PDF’lerini belge
          detayından indirebilirsin.
        </Text>
        <Button
          ...
          onPress={() =>
            void run(async () => {
              const file = new File(Paths.cache, "aldim-kayitlarim.json");
              file.write(
                JSON.stringify(
                  {
                    version: 3,
